...
void statement() /* 문 */
{
  ...
  top_line = Pc; end_line = code.jumpAdress;          /* 제어 범위의 시작과 끝 */
...
CodeSet nextCode() 						/* 코드 획득 */
{
  ...
  case While: case For: case If: case Elif: case Else:
       jumpAdress = *SHORT_P(code_ptr); code_ptr += SHORT_SIZ;
       return CodeSet(kd, -1, jumpAdress);          // 점프할 주소 
  case String:
